...
$(document).ready(function() {
 $("#book_id").on("change", function() {
  window.location = '/books/'+ $(this).val()
    });
  });

 // Autocomplete Booksearch

var ready;
ready = function() {


var titleTokens = new Bloodhound ({
  ...
  prefetch: {
    ...
    filter: function(books) {
      return $.map(books, function(book) { return { amazon_url: book.amazon_url, id: book.id, title: book.title, isbn: book.isbn, author: book.author, small_image_url: book.small_image_url, medium_image_url: book.medium_image_url };
    });
  }
 }
});

 var authorTokens = new Bloodhound ({
datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.author); },
  queryTokenizer: Bloodhound.tokenizers.whitespace,
  limit: 10,
  prefetch: {
    ttl: 1,
...
var isbnTokens = new Bloodhound ({
datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.isbn); },
  queryTokenizer: Bloodhound.tokenizers.whitespace,
  limit: 10,
  prefetch: {
    ttl: 1,
...
// initialize bloodhound; kicks off the loading/processing of `local` and `prefetch`
titleTokens.initialize();
authorTokens.initialize();
isbnTokens.initialize();

$('#scrollable-dropdown-menu .typeahead').typeahead({/* pass in your datasets and initialize the typeahead */
    hint: true,
    highlighter: true,
    minLength: 2
  },
  {
    name: 'title',
    displayKey: 'title',
    source: titleTokens.ttAdapter()
},

{
    name: 'isbn',
    displayKey: 'title',
    source: isbnTokens.ttAdapter()
},

{
    name: 'author',
    displayKey: 'title',
    source: authorTokens.ttAdapter()
})
...
function onAutocompleted($e, datum) {
  ...
  console.log(datum);
  $('#scrollable-dropdown-menu').on('typeahead:selected', function(e, datum){
  console.log(datum.title);
  $('.typeahead').val(datum.title);
});
  $('.btn-amazon').hide();
  $('#scrollable-dropdown-menu input:text').focus(
                  function(){
                $('.typeahead').val('');
                $('.btn-amazon').show();
              });
...
function onChange() { // user typed their own value
  value = $('#scrollable-dropdown-menu .btn-amazon').on('change', function(e) {
    ...
    dataType: 'json',
        success: function (response) {    
          if (response) {
            ...
            $('#scrollable-dropdown-menu').addClass('scrollable-dropdown-menu title').append('<ul>'+ this.title+ '</ul>');
              } else {
              $('#scrollable-dropdown-menu').addClass('scrollable-dropdown-menu title').append('<ul>'+'<img src="' + this.small_image_url + ' " />' + '<p>' +this.title+ '</p>' + '</ul>');
              }

  $('#scrollable-dropdown-menu ul:last').on('click', function(e){
    $('#scrollable-dropdown-menu ul').hide();
    $('.btn-amazon').hide();
    $('.typeahead').val(i.title);
  var postData = {
    "isbn": i.isbn,
    "amazon_url": i.amazon_url,
    "title": i.title,
    "author": i.author,
    "medium_image_url": i.medium_image_url,
...
// Save book to database.
  ...
  $.ajax({
              ...
              success: function(postData, textStatus, jqXHR){
                  ...
                  console.log(i.title);
              e.preventDefault();
              e.stopPropagation(); //e not defined. add e to function
            }
          });
        }); 
      })
                ...
                $('.typeahead').val('');
                $('.btn-amazon').show();
                $('#scrollable-dropdown-menu ul').empty();
              });
...
$(document).ready(function(){
    ...
    function onPlayClick(playButton, player) {
        if (player.paused) {
            player.play();
            $(playButton).html('<span class="glyphicon glyphicon-pause"></span> pause');
        } else {
            player.pause();
            $(playButton).html('<span class="glyphicon glyphicon-play"></span> play');
        }
    }
...
$('#scrollable-dropdown-menu .theme-typeahead').typeahead({/* pass in your datasets and initialize the typeahead */
    hint: true,
    highlighter: true,
    minLength: 1
 },
...
// .blur(function(){
// match = false
// for (var i = keywordTokens.index.datums.length - 1; i >= 0; i--) {
//   if($(this).val() == keywordTokens.index.datums[i].name){
//     match = true;
//   }
// };
// if(!match){
...
// $('#scrollable-dropdown-menu').on('typeahead:selected typeahead:autocompleted', function(e,datum) {
// $(this).parent().parent().first('.theme_id').val(datum.id);
//  });
...
// get value
 ...
 //          tokens: [data[i].keyword],
 //          keyword: data[i].keyword,
 //          id: data[i].id,
 //          template: '<p>{{keyword}}</p>',
 //        });
 //      }
 //      return retval;
 ...
 function saveTheme () {
  ...
  $.ajax({
    ...
    // contentType: 'application/json',
    dataType: 'json',
    //data: $('#scrollable-dropdown-menu').serialize(),
    data: JSON.stringify(postData),
    // success: function(data, textStatus, jqXHR){
     // $('#scrollable-dropdown-menu').append(''+ datum.keyword + '');
     success: function(data, textStatus, jqXHR){
